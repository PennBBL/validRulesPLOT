...
df2 <- merge(df, demo) #, by=c("test_sessions.", "test_sessions_v.battery", "test_sessions_v.dotest", "test_sessions.datasetid"))

# Remove invalid data ("N")
df2 <- df2[df2$test_sessions_v.valid_code != "N",]

# Just select subjects who have come in since September 2017
df2$dotest <- as.Date(df2$test_sessions_v.dotest, format="%Y-%m-%d")
df3 <- df2[df2$dotest >= "2017-04-25",]

# Prepare dataframes
df_1515 <- df3[!(is.na(df3$VSPLOT15.VSPLOT15_CR)), c("test_sessions.subid", "test_sessions_v.valid_code", "VSPLOT15.VSPLOT15_CR", "VSPLOT15.VSPLOT15_SUM_DEG_OFF", "VSPLOT15.VSPLOT15_RTCR", "test_sessions_v.age")]
df_2424 <- df3[!(is.na(df3$VSPLOT24.VSPLOT24_CR)), c("test_sessions.subid", "test_sessions_v.valid_code", "VSPLOT24.VSPLOT24_CR", "VSPLOT24.VSPLOT24_SUM_DEG_OFF", "VSPLOT24.VSPLOT24_RTCR", "test_sessions_v.age")]
df_2415 <- df3[!(is.na(df3$VSPLOT24.VSPLOT15_CR)), c("test_sessions.subid", "test_sessions_v.valid_code", "VSPLOT24.VSPLOT15_CR", "VSPLOT24.VSPLOT15_SUM_DEG_OFF", "VSPLOT24.VSPLOT15_RTCR", "test_sessions_v.age")]
...
m_age_1515 <- round(mean(df_1515$Age), digits=2)
sd_age_1515 <- round(sd(df_1515$Age), digits=2)
m_age_2415 <- round(mean(df_2415$Age), digits=2)
sd_age_2415 <- round(sd(df_2415$Age), digits=2)
m_age_2424 <- round(mean(df_2424$Age), digits=2)
sd_age_2424 <- round(sd(df_2424$Age), digits=2)
...
p_CR_1515 <- ggplot(df_1515, aes(x=CR)) + theme_minimal() +
	geom_histogram(position="identity", alpha=0.5, colour="violetred4", fill="violetred3", bins=(max(df_1515$CR)-min(df_1515$CR)+1)) + 
...
p_CR_2415 <- ggplot(df_2415, aes(x=CR)) + theme_minimal() +
	geom_histogram(position="identity", alpha=0.5, colour="steelblue4", fill="steelblue3", bins=(max(df_2415$CR)-min(df_2415$CR)+1)) + 
...
p_CR_2424 <- ggplot(df_2424, aes(x=CR)) + theme_minimal() +
	geom_histogram(position="identity", alpha=0.5, colour="darkseagreen4", fill="darkseagreen3", bins=(max(df_2424$CR)-min(df_2424$CR)+1)) + 
	labs(title="Total Correct for VSPLOT 24",x="# Correct", y = "# of Participants", subtitle=sub_CR_2424) +
	theme(plot.title = element_text(size=15), axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))

# Plot percent correct
m_per_1515 <- 100*round(mean(df_1515$CR/15), digits=3)
sd_per_1515 <- 100*round(sd(df_1515$CR/15), digits=3)
N_per_1515 <- nrow(df_1515)
df_1515$per_CR <- 100*(df_1515$CR/15)
sub_per_1515 <- paste0("(Mean = ", m_per_1515, ", SD = ", sd_per_1515, ", N = ", N_per_1515, ")", agestring_1515)
p_per_1515 <- ggplot(df_1515, aes(x=per_CR)) + theme_minimal() +
	geom_histogram(position="identity", alpha=0.5, colour="violetred4", fill="violetred3", bins=(max(df_1515$CR)-min(df_1515$CR)+1)) + 
	labs(title="Percent Correct for VSPLOT 15 (Original)",x="% Correct", y = "# of Participants", subtitle=sub_per_1515) +
	theme(plot.title = element_text(size=15), axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))

m_per_2415 <- 100*round(mean(df_2415$CR/15), digits=3)
sd_per_2415 <- 100*round(sd(df_2415$CR/15), digits=3)
N_per_2415 <- nrow(df_2415)
df_2415$per_CR <- 100*(df_2415$CR/15)
sub_per_2415 <- paste0("(Mean = ", m_per_2415, ", SD = ", sd_per_2415, ", N = ", N_per_2415, ")", agestring_2415)
p_per_2415 <- ggplot(df_2415, aes(x=per_CR)) + theme_minimal() +
	geom_histogram(position="identity", alpha=0.5, colour="steelblue4", fill="steelblue3", bins=(max(df_2415$CR)-min(df_2415$CR)+1)) + 
	labs(title="Percent Correct for VSPLOT 15 (Derived)",x="% Correct", y = "# of Participants", subtitle=sub_per_2415) +
	theme(plot.title = element_text(size=15), axis.text=element_text(size=12),axis.title=element_text(size=14,face="bold"))

m_per_2424 <- 100*round(mean(df_2424$CR/24), digits=3)
sd_per_2424 <- 100*round(sd(df_2424$CR/24), digits=3)
N_per_2424 <- nrow(df_2424)
df_2424$per_CR <- 100*(df_2424$CR/24)
sub_per_2424 <- paste0("(Mean = ", m_per_2424, ", SD = ", sd_per_2424, ", N = ", N_per_2424, ")", agestring_2424)
p_per_2424 <- ggplot(df_2424, aes(x=per_CR)) + theme_minimal() +
	geom_histogram(position="identity", alpha=0.5, colour="darkseagreen4", fill="darkseagreen3", bins=(max(df_2424$CR)-min(df_2424$CR)+1)) + 
...
# Return pdf
pdf("/home/butellyn/plot_validation/plots/distributions_CRdegoffmedtime_libi.pdf", width=18, height=5)
